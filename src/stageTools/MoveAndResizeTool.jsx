...
import { stageToContainerCoordinates, containerToStageCoordinates } from "../shared/utilities.js";
...
const MoveAndResizeToolInner = ({ containerElement }) => {
    const [[layers, selectedLayerIds, stageTransform], dispatch] = useOverlayEditorContext(
    ...
    const onMouseDown = useCallback((evt) => {
        ...
        const onBoundingBoxChanged = (newRect, trackUndo) => {
            ...
            dispatch("UpdateSelectedLayersProps", (layer) => {
                const originalSelectedRect = originalSelectedRects.find(r => r.layerid == layer.id);
                return {
                    top: newRect.top + ((originalSelectedRect.top - originalBoundingBox.top) * scaleHeight),
                    left: newRect.left + ((originalSelectedRect.left - originalBoundingBox.left) * scaleWidth),
