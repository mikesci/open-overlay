...
import { FormGroup, InputGroup, TextArea, Switch, HTMLSelect, RadioGroup, Radio, Slider, Collapse, Button, Popover, ButtonGroup, Icon } from "@blueprintjs/core";
...
export default class ConfigurationForm extends React.Component {
    ...
    onAssetUrlDrop = (evt) => {
        evt.preventDefault();
        evt.stopPropagation();
        const parameterName = evt.target.getAttribute("data-param");
        this.handleUpload(evt.dataTransfer.files, parameterName);
        
    }

    onUploadClick = (evt) => {
        const buttonTarget = evt.target.closest("button");
        const parameterName = buttonTarget.getAttribute("data-param");
        const input = document.createElement("input");
        input.type = "file";
        input.style = "display: none";
        input.accept = evt.target.getAttribute("accept");
        input.addEventListener("change", (evt) => {
            if (input.files.length > 0) {
                this.handleUpload(input.files, parameterName);
            }
        });
        input.click();
    }

    handleUpload = async (files, parameterName) => {
        let layers = await DataTransferManager.handleFileUpload(files, this.props.onUpload);

        // should only ever return one layer
        if (layers.length == 1) {
            const layer = layers[0];
            // copy the parameter name from the layer config returned
            let obj = { [parameterName]: layer.config[parameterName] };
            this.props.onParameterValuesChanged(obj, true);
        }
    }
    ...
    renderInputGroup = (parameter, value) => {
        ...
        switch (parameter.type) {
            ...
            case "assetUrl":
                return (<InputGroup value={value} type="text" onChange={this.onFieldChanged} onFocus={this.onTextboxFocus} onBlur={this.onTextboxBlur} data-param={parameter.name} fill={true} onDragOver={this.onAssetUrlDragOver} onDrop={this.onAssetUrlDrop} rightElement={<Button icon="cloud-upload" minimal={true} onClick={this.onUploadClick} data-param={parameter.name} accept={parameter.accept} />} />);
            case "textarea":
